...
//        panel1.add(l10);
    ...
    public void display() {
        try {
            Conn conn = new Conn();
            PreparedStatement ps = conn.c.prepareStatement("select booking_id, place, name, customerID_Number, persons, date, totalprice from bookpackages where booking_id=?");

            ps.setLong(1, id);
            ResultSet rs = ps.executeQuery();

            if (rs.next()) {
                l3.setText(rs.getString(1)); // booking_id
                l4.setText(rs.getString(2)); // place
                l22.setText(rs.getString(3)); // name
                l7.setText(rs.getString(4)); // customerID_Number
                l17.setText(rs.getString(5)); // persons
                l15.setText(rs.getString(6)); // date
                l13.setText(rs.getString(7)); // totalprice
...
//                new AddPackage(this, place).setVisible(true);
            ...
            if (c != null) {
                BookedPackagePanel bp = new BookedPackagePanel(c, user);
                add(bp);
                bp.setVisible(true);
            }
        }
