...
public class AddPackage extends JFrame implements ActionListener {
    ...
    AddPackage(String adminUsername) {
        this();
    ...
    public AddPackage(PackageDetails pd, String place) {
        ...
        try {
            Conn conn = new Conn();
            String sql = "select *from  packages where place='" + place + "'";
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == b1) {
            ...
            String placeName = t1.getText();
            String cityStateName = t2.getText();
            String price = t3.getText();
            String daysNights = t4.getText();
            String description = ta1.getText();

            if (placeName.isEmpty() || cityStateName.isEmpty() || price.isEmpty() || daysNights.isEmpty() || description.isEmpty()) {
            ...
            // Validate daysNights field
            if (!daysNights.matches("[\\w\\s]+")) {
                JOptionPane.showMessageDialog(null, "Days and Nights must contain numbers, spaces, and string characters.");
                return; // Stop further execution
            }
            ...
            if (photo.length > 300 * 1024) { // Check if photo size exceeds 300KB
                JOptionPane.showMessageDialog(null, "Photo size must be up to 300KB.");
                return; // Stop further execution
            }

            try {
                Conn conn = new Conn();
                int loggedInAdminId = getCurrentAdminId();
                String sql = "INSERT INTO packages (place, state, price, days_nights, description, image, created_by) values(?, ?, ?, ?, ?, ?, ?)";
                PreparedStatement ps = conn.c.prepareStatement(sql);
            ...
            try {
                ...
                Conn conn = new Conn();
                String sql1 = "update packages set place=?, state=?, price=?, days_nights=?, description=?, image=? where place='" + place + "'";
