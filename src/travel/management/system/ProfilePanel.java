	...
	JLabel l1, l2, l3, l4, l5, l6, l7;
        ...
        public ProfilePanel(AdminHome a, String username){
            ...
            this.a = a;
                try{
                    Conn conn = new Conn();
                    String sql = "select * from adminlogins where username = '"+username+"'";
        ...
        public ProfilePanel(CustomerHome c, String user){
            ...
            this.c = c;
                try{
                    ...
                    ResultSet rs = conn.s.executeQuery(sql);
                    if(rs.next()){
                        t1.setText(rs.getString("full_name"));
                        t2.setText(rs.getString("username"));
                        t3.setText(rs.getString("email"));
                        t4.setText(rs.getString("number"));
//                        t3.setText(rs.getString("password"));
...
//                t6.setEditable(false);
            ...
            if(ae.getSource() == b3){
                ...
                try{
                    Conn conn = new Conn();
                    String sq = "select * from adminlogins where username = '"+username+"'";
                    ResultSet rs1 = conn.s.executeQuery(sq);
                    if(rs1.next()){
                    String sql = "update adminlogins set name=?, username=?, email=?, number=? where username ='"+username+"'";
...
//                    ps.setString(6, t6.getText());
                    ...
                    b2.setVisible(false);
                    b3.setVisible(false);
//                    String sql1 = "update adminlogins set username=?, password=? where username ='"+username+"'";
...
//                    }
                ...
                try{
                    Conn conn = new Conn();
                    String sq = "select * from users where username = '"+user+"'";
                    ResultSet rs1 = conn.s.executeQuery(sq);
                    if(rs1.next()){
                    String sql = "update users set full_name=?, username=?, email=?, number=? where username ='"+user+"'";
                    PreparedStatement ps = conn.c.prepareStatement(sql);
                    ps.setString(1, t1.getText());
                    ps.setString(2, t2.getText());
                    ps.setString(3, t3.getText());
...
//                    ps.setString(6, t6.getText());
                    ...
                    b2.setVisible(false);
                    b3.setVisible(false);
//                    String sql1 = "update accounts set name=?, username=?, password=? where username ='"+user+"'";
//                    PreparedStatement ps1 = conn.c.prepareStatement(sql1);
